...
@Component({
  selector: 'app-nove-rezervacije',
  templateUrl: './nove-rezervacije.component.html',
  styleUrls: ['./nove-rezervacije.component.css']
})
export class NoveRezervacijeComponent implements OnInit {
      ...
      this.servis.getDetaljiFirme(this.ulogovan.kor_ime).subscribe((res2:DetaljiFirme)=>{
        ...
        if (this.ulogovan.tip=='ski_skola'){
          this.servis.dohvRezervacijeSkole(this.ulogovan._id).subscribe((res3:RezSki[])=>{
            ...
            if(this.showAlertErr.length == 0){
              this.nema_rez_ski = true;
            }else{
              this.nema_rez_ski = false;
            }
          })
                ...
                this.sveTure.forEach(tura => {
                  if(tura._id == rezPlan.idTure){
                    ...
                    rezPlan.rezervacije.forEach(rez => {
                      ...
                      this.rezTura.push({naslov: tura.naslov, rezervacija: rez});
                    });
                  }
                });
              });
              console.log("naslovi", this.naslovTure)
              console.log("rezTura", this.rezTura[0]['naslov']);
            })
          })
        }
      })
  ...
  prihvatiRezSki(idRez, index){
    this.servis.prihvatiRezSki(idRez).subscribe((res:any)=>{
      console.log(res);
  ...
  prihvatiRezTure(idRez, index){
    this.servis.prihvatiRezTura(idRez).subscribe((res:any)=>{
      console.log(res);
      this.message = res.message;
      this.showAlertErr[index] = false;
      this.showAlertOK[index] = true;
  ...
  prihvatiRezSmestaja(idRez, index){
    this.servis.prihvatiRezSmestaj(idRez).subscribe((res:any)=>{
      console.log(res);
      this.message = res.message;
      this.showAlertErr[index] = false;
      this.showAlertOK[index] = true;
